...
sample <- pomp(sampleData, time="time", t0=0, 
...
simulate(sample,params=mle,nsim=10,as.data.frame=TRUE,include.data=TRUE) %>%
...
simulate(sample,params=theta,nsim=10,as.data.frame=TRUE,include.data=TRUE) %>%
  ggplot(mapping=aes(x=time,y=Cases,group=sim,alpha=(sim=="data")))+
  scale_alpha_manual(name="",values=c(`TRUE`=1,`FALSE`=0.2),
                     labels=c(`FALSE`="simulation",`TRUE`="data"))+
  geom_line()
