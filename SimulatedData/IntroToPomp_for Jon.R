...
ggplot(data=data, mapping= aes(x=time, y= Cases))+
...
## example simulation at the true values
simStates <- simulate(po,nsim=5,params=true_parameters,as.data.frame=T)


## plot the simulation in black it's the actual cases we see in colour is the simulations i just got

p<-ggplot(data=simStates, mapping= aes(x=time, y= Cases, col= sim))
p<-p+geom_line()+geom_point(data=data,aes(x=time,y=Cases), color="black")



coef(po)<-c(beta_sd= 0.01, beta1= 2,mu=0.2, gamma=0.6,rho= 0.8, N=N, S.0= S.0, I.0 = I.0, RealCases.0 = 1)


# check that the transforms are working
print(coef(po, transform=TRUE))

 
# filtering!
 
pf <- pfilter(po,Np=1000)
print(logLik(pf)) # evaluates the filtering
...
pars <- parmat(c(beta_sd= 0.1, beta1= 2,mu=0.2, gamma=0.6,rho= 0.8, N=N, S.0= S.0, I.0 = I.0, RealCases.0 = 1),5)

pars["beta1",] <- seq(0.7,2,length=5)
 
traj <- trajectory(po,params=pars,times=seq(0,nrow(data),by=0.01), as.data.frame=TRUE)

p1<-ggplot()+geom_line(data=traj,aes(x=time,y=S,color=traj))
 
p2<-ggplot()+geom_line(data=traj,aes(x=time,y=RealCases,color=traj))  
...
tm <- traj.match(po,start=pars[,1],
                 est=c("beta_sd", "beta1", "mu", "gamma",
                       "rho","N", "S.0", "I.0"),
                 transform=TRUE)
coef(po)


tm_params <- coef(tm,transform=F)
...
## plot end result of trajectory matching with data
traj <- trajectory(tm,as.data.frame=TRUE)

p3<-ggplot()+geom_line(data=traj,aes(x=time,y=tm_params["rho"]*RealCases)) +
